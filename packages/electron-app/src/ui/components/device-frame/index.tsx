		...
		<>
			<div id={`logsTab`} className={`${clicked ? "expandBar" : ""}`} css={statusBarContainerStyle}>
				<div
					...
					`}
				>
					<div onClick={setClicked.bind(this, !clicked)} css={statusBarTabStyle}>
						Logs{" "}
					...
					<Conditional showIf={!clicked}>
						<div css={logTextStyle} className={"ml-20"}>
							{logs && logs.length ? logs[logs.length - 1].message : ""}
						</div>
					</Conditional>
				...
				<Conditional showIf={clicked}>
					...
					>
						{logs &&
							logs.map((log: ILoggerReducer["logs"][0], index: number) => {
		...
		>
			<div css={containerStyle}>
				<Conditional showIf={!!recorderInfo.device}>
					<div
						style={{
							width: `${recorderInfo.device?.width}rem`,
							height: `${recorderInfo.device?.height}rem`,
							maxWidth: "100%",
							maxHeight: "100%",
							position: "relative",
						}}
